...
try {
    ...
    if (!function_exists('app_ensure_student_attendance_schema')) {
        function app_ensure_student_attendance_schema(PDO $pdo): void
        {
            try {
                $pdo->exec("CREATE TABLE IF NOT EXISTS student_attendance_settings (
            ...
            try {
                $pdo->exec("CREATE TABLE IF NOT EXISTS student_attendance_records (
            ...
            // Jadwal absen manual per rentang waktu.
            try {
                $pdo->exec("CREATE TABLE IF NOT EXISTS student_attendance_windows (
                    id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
                    name VARCHAR(100) NOT NULL,
            ...
            // Relasi jadwal absen -> siswa yang diwajibkan/diizinkan absen.
            try {
                $pdo->exec("CREATE TABLE IF NOT EXISTS student_attendance_window_students (
                    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
            ...
            try {
                $pdo->exec("CREATE TABLE IF NOT EXISTS student_attendance_change_requests (
                    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    if (!function_exists('app_ensure_site_comments_schema')) {
        function app_ensure_site_comments_schema(PDO $pdo): void
        {
            ...
            // Backward-compatible patch: add parent_id + index on existing installs.
            try {
                $hasParentCol = false;
                try {
                    $hasParentCol = (bool)$pdo->query("SHOW COLUMNS FROM site_comments LIKE 'parent_id'")->fetch();
                } catch (Throwable $eCol) {
                    $hasParentCol = false;
                }

                if (!$hasParentCol) {
                    $pdo->exec("ALTER TABLE site_comments ADD COLUMN parent_id BIGINT UNSIGNED NULL AFTER id");
                }

                $hasIndex = false;
                try {
                    $q = $pdo->prepare("SHOW INDEX FROM site_comments WHERE Key_name = :k");
                    $q->execute([':k' => 'idx_site_comments_page_parent']);
                    $hasIndex = (bool)$q->fetch();
                } catch (Throwable $eIdx) {
                    $hasIndex = false;
                }

                if (!$hasIndex) {
                    $pdo->exec("ALTER TABLE site_comments ADD KEY idx_site_comments_page_parent (page_identifier, parent_id, id)");
                }
            } catch (Throwable $e) {
                // ignore
            }
