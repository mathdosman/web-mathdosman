...
if (!$message && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // (Seed demo sudah dihapus)

        // Auto-seed konten materi bawaan (best-effort, idempotent).
        try {
            $hasContents = (bool)$pdo->query("SHOW TABLES LIKE 'contents'")->fetchColumn();
            if ($hasContents) {
                require_once __DIR__ . '/../scripts/seed_content_barisan_geometri_01.php';
                if (function_exists('seed_content_barisan_geometri_01')) {
                    seed_content_barisan_geometri_01($pdo, [
                        'skip_if_exists' => true,
