...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Endpoint untuk menyimpan skor game (dipanggil via fetch).
    ...
    try {
        ...
        $stmt->execute([
            ':sid' => $studentId,
            ':name' => (string)($student['nama_siswa'] ?? ''),
        ...
        function endGame() {
            ...
            if (score > 0 && csrfToken) {
                ...
                fetch(window.location.href, {
                    ...
                    // Sengaja diabaikan; admin akan melihat skor di sisi backend.
                }).catch(function() {
                    // Abaikan error jaringan, tidak boleh mengganggu UI siswa.
                });
            }

            // Tampilkan SweetAlert dengan skor dan tombol ulangi bermain (khusus siswa login).
