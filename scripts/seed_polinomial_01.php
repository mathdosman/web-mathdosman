...
// Jalankan via CLI: php scripts/seed_polinomial_01.php

/**
 * @return array{ok:bool, message:string, skipped?:bool, package_id?:int}
 */
function seed_package_polinomial_01(PDO $pdo, array $options = []): array
{
...
if (PHP_SAPI === 'cli' && realpath((string)($_SERVER['SCRIPT_FILENAME'] ?? '')) === __FILE__) {
    ...
    $result = seed_package_polinomial_01($pdo, ['skip_if_exists' => false]);
    if (!($result['ok'] ?? false)) {
    ...
    if (!empty($result['package_id'])) {
        echo 'Open: paket.php?code=' . rawurlencode('Polinomial-01') . "\n";
    }
}
