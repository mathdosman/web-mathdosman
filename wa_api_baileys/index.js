...
const requireWaToken = (req, res, next) => {
    if (!WA_API_TOKEN) {
        return next();
    }

    const headerToken = req.headers['x-wa-token'] || req.get('X-WA-TOKEN');
    if (!headerToken || headerToken !== WA_API_TOKEN) {
...
app.post("/send-message", requireWaToken, async (req, res) =>{
    //console.log(req);
...
// send group message
app.post("/send-group-message", requireWaToken, async (req, res) =>{
    //console.log(req);
    const pesankirim = req.body.message;
